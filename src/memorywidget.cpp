...
void MemoryWidget::paintEvent(QPaintEvent* event)
{
  ...
  QRect rect4(fieldHexX, 0, (fieldAsciiX - fieldHexX), event->rect().bottom() + 1);
  ...
  rect2 = QRect(fieldAsciiX, HEADER_HEIGHT + 1, PAD_ASCII_LEFT + charWidth * BYTES_PER_ROW + PAD_ASCII_RIGHT, event->rect().bottom());
  painter.fillRect(rect2, background2);
