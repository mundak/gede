...
void TagScanner::checkForCtags()
{
  ...
  if (!g_ctagsExist)
  {
    ...
    QMessageBox::warning(NULL, "Failed to start ctags", msg);
  }
  else
  {
...
int TagScanner::execProgram(QString name, QStringList argList, QByteArray* stdoutContent, QByteArray* stderrContent)
{
...
int TagScanner::scan(QString filepath, QList<Tag>* taglist)
{
  ...
  int rc = execProgram(g_ctagsCmd, argList, &stdoutContent, &stderrContent);
