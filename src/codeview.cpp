...
bool isLegalExpression(QString name)
{
  if (name.isEmpty())
    return false;
  if (name[0].isLetter() || name[0] == '_' || name[0] == ':' || name[0] == '&')
  {
    return true;
  }
  return false;
  ...
  if (event->button() == Qt::RightButton)
  {
    ...
    if (rowIdx >= 0 && rowIdx < (int) m_highlighter->getRowCount())
    {
      ...
      if (foundPos != -1)
      {
        ...
        while (foundPos >= 0)
        {
          if (cols[foundPos]->isSpaces() || m_highlighter->isKeyword(cols[foundPos]->m_text) || m_highlighter->isSpecialChar(cols[foundPos]))
          {
            foundPos--;
          }
          else
...
int CodeView::doIncSearch(QString pattern, int startPos, bool searchForward)
{
  ...
  else
  {
    ...
    debugMsg("Found search term '%s' at L%d:%d", qPrintable(pattern), m_incSearchStartPosRow + 1, m_incSearchStartPosColumn);
  }
